...
finish_packet:
			mouse_cycle = 0;
			if (mouse_byte[0] & 0x80 || mouse_byte[0] & 0x40) goto read_next;
			...
			if (mouse_byte[0] & 0x01) packet.buttons |= LEFT_CLICK;
			if (mouse_byte[0] & 0x02) packet.buttons |= RIGHT_CLICK;
			if (mouse_byte[0] & 0x04) packet.buttons |= MIDDLE_CLICK;
			if (mouse_mode == MOUSE_SCROLLWHEEL && mouse_byte[3]) {
				if (mouse_byte[3] > 0) packet.buttons |= MOUSE_SCROLL_DOWN;
				else if (mouse_byte[3] < 0) packet.buttons |= MOUSE_SCROLL_UP;
			}
...
void mouse_wait(uint8_t a_type) {
	...
	if (!a_type) {
		...
		while (--timeout) {
			if (!((port_byte_in(MOUSE_STATUS) & MOUSE_ABIT))) return;
		}
		return;
	}
}
...
uint8_t mouse_read(void) {
	mouse_wait(0);
	char t = port_byte_in(MOUSE_PORT);
	return t;
}
...
void init_mouse() {
	...
	register_interrupt_handler(IRQ12, (isr_t) mouse_handler);

	uint8_t tmp = port_byte_in(0x61);
	port_byte_out(0x61, tmp | 0x80);
	port_byte_out(0x61, tmp & 0x7F);
