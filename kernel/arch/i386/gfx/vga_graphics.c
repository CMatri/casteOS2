// THIS DRIVER DOES NOT CURRENTLY WORK
// Need to map VGA_VIDEO_ADDRESS into virtual memory most likely. I never use this mode but I should get off my lazy ass and fix it.
...
void vga_print_at(char *message, int col, int row) { vga_print_at_color(message, col, row, WHITE_ON_BLACK); }
...
void vga_clear_screen() {
    ...
    int i;
    uint8_t *screen = (uint8_t*) VGA_VIDEO_ADDRESS;
...
int print_char(char c, int col, int row, char attr) {
    uint8_t *vidmem = (uint8_t*) VGA_VIDEO_ADDRESS;
    if (!attr) attr = WHITE_ON_BLACK;

    /* Error control: print a red 'E' if the coords aren't right */
    if (col >= VGA_MAX_COLS || row >= VGA_MAX_ROWS) {
        vidmem[2*(VGA_MAX_COLS)*(VGA_MAX_ROWS)-2] = 'E';
        vidmem[2*(VGA_MAX_COLS)*(VGA_MAX_ROWS)-1] = RED_ON_WHITE;
    ...
    if (offset >= VGA_MAX_ROWS * VGA_MAX_COLS * 2) {
        int i;
        for (i = 1; i < VGA_MAX_ROWS; i++) memcpy((uint8_t*)(get_offset(0, i-1) + VGA_VIDEO_ADDRESS), (uint8_t*)(get_offset(0, i) + VGA_VIDEO_ADDRESS), VGA_MAX_COLS * 2);

        /* Blank last line */
        char *last_line = (char*) (get_offset(0, VGA_MAX_ROWS-1) + (uint8_t*) VGA_VIDEO_ADDRESS);
